...
Agent API keys are generated by the server during registration and start with
`hc_`. User endpoints require the config `secret_key` as the Bearer token.
...
Returns the agent's API key (shown once).
...
```http
POST /api/knowledge/facts?key=preferred_editor&value=vim&source=agent_insight
Authorization: Bearer <agent_api_key>
```
...
```http
POST /api/knowledge/corroborate?key=preferred_editor&value=vim
Authorization: Bearer <agent_api_key>
```
